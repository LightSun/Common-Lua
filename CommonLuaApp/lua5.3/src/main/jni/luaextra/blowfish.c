...
for (int i = 0; i < count; ++i) {\
    ...
    BYTE * onn = (BYTE *) out;\
    if(en){\
        blowfish_encrypt((const BYTE *) (inn + BLOWFISH_BLOCK_SIZE * i), (onn + BLOWFISH_BLOCK_SIZE * i), ctx);\
    }else{\
        blowfish_decrypt((const BYTE *) (inn + BLOWFISH_BLOCK_SIZE * i), (onn + BLOWFISH_BLOCK_SIZE * i), ctx);\
    }\
}

void BF_de(BLOWFISH_KEY *ctx, const char *key, int keyLen, const char *in, char *out, int inLen) {
    blowfish_key_setup((const BYTE *) key, ctx, (size_t) keyLen);

    int count = inLen / BLOWFISH_BLOCK_SIZE;
    bf_do(0);
}
...
void BF_en(BLOWFISH_KEY *ctx, const char *key, int keyLen, const char *in, char *out, int inLen) {
    ...
    int count = inLen / BLOWFISH_BLOCK_SIZE;
    bf_do(1);
    int left = inLen % BLOWFISH_BLOCK_SIZE;
    if (left > 0) {
        char buf[BLOWFISH_BLOCK_SIZE];
        int start = count * BLOWFISH_BLOCK_SIZE;
        //copy left to buf
        for (int i = start; i < inLen; ++i) {
            buf[i - start] = in[i];
        }
        for (int i = left; i < BLOWFISH_BLOCK_SIZE; ++i) {
            buf[i] = 0;
        }
        BYTE *onn = (BYTE *) out;
        blowfish_encrypt((const BYTE *) buf, (onn + BLOWFISH_BLOCK_SIZE * count), ctx);
    }
}
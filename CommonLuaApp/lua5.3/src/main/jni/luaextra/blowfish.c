...
void BF_de(BLOWFISH_KEY *ctx, const char* key, int keyLen, const char* in, char* out, int len){
    ...
    int count = len / BLOWFISH_BLOCK_SIZE;
    for (int i = 0; i < count; ++i) {
        BYTE * inn = (BYTE *) in;
        BYTE * onn = (BYTE *) out;
        blowfish_decrypt((const BYTE *) (inn + 8 * i), (onn + 8 * i), ctx);
    }
}
...
void BF_en(BLOWFISH_KEY *ctx, const char* key, int keyLen, const char* in, char* out, int inLen){
    ...
    for (int i = 0; i < count; ++i) {
        BYTE * inn = (BYTE *) in;
        BYTE * onn = (BYTE *) out;
        blowfish_encrypt((const BYTE *) (inn + 8 * i), (onn + 8 * i), ctx);
    }
    if(left > 0){
        ...
        BYTE * onn = (BYTE *) out;
        blowfish_encrypt((const BYTE *) buf, (onn + 8 * count), ctx);
    }
